...
public class Tela004 extends AppCompatActivity implements ProdutoAdapter.OnClickProdutoListener {

    ImageButton ib_pesquisa;
    ImageButton ib_carrinho;
    ImageButton ib_home;
    TextView tv_prod_pesq;
    TextView tv_merc_pesq;
    String comando = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv_merc_pesq = findViewById(R.id.tv_merc_pesq);
    ...
    public void onClick_ib_home(View v) {
        Intent it_trocaTela = new Intent(Tela004.this, Principal.class);
    ...
    public void onClick_ib_carrinho(View v) {
        Intent it_trocaTela = new Intent(Tela004.this, Tela005.class);
        startActivity(it_trocaTela);
    }
    ...
    public void onClick_ib_search(View v) {
        ...
        String buscaprod = tv_prod_pesq.getText().toString();

        String buscamerc = tv_merc_pesq.getText().toString();
        if (!buscaprod.isEmpty() && !buscamerc.isEmpty()) {
            try {
                ResultSet resultSet = banco.select("SELECT produto.cod_produto, produto.marca ,produto.tipo ,produto.peso ,produto.caracteristicas ,produto.preco ,produto.qtd_estoque ,mercado.cod_mercado, mercado.nome_mercado  FROM produto natural join mercado where produto.tipo='"+buscaprod+"' and mercado.nome_mercado='"+buscamerc+"'");
                if (resultSet != null) {
                    while (resultSet.next()) {
                        Produto obj = new Produto();
                        obj.setPrice(resultSet.getFloat("preco"));
